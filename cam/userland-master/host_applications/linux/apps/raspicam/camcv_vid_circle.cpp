...
static void default_status(RASPIVID_STATE *state)
{
	...
	state->timeout 			= 6000000;     // capture time : here 65 s
...
static void video_buffer_callback(MMAL_PORT_T *port, MMAL_BUFFER_HEADER_T *buffer)
{
	...
	if (pData)
	{
		...
		if (buffer->length)
		{
			...
			// detect circles
			//HoughCircles(gray, circles, CV_HOUGH_GRADIENT, 2, gray.rows/4, 200, 115);
			// detect faces
...
void make_hsv_adjustment() {
	...
	cvCreateTrackbar("LowS", "Control", &iLowS, 0); //Saturation (0 - 255)
	cvCreateTrackbar("HighS", "Control", &iHighS, 255);

	cvCreateTrackbar("LowV", "Control", &iLowV, 0); //Value (0 - 255)
