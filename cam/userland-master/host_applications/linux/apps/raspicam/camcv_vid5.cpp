	...
	if (pData)
	{
		...
		if (buffer->length)
		{
			...
			{       
				...
				//snprintf((char*)message.data(), 50, "%04d :)", tmpGlobalX);
				*((int*)message.data())     = face_i.x;
				*(((int*)message.data())+1) = face_i.y;
				*(((int*)message.data())+2) = face_i.width;
				*(((int*)message.data())+3) = face_i.height;
				//snprintf((char*)message.data(), 50, "gby");
				//sprintf((char*)message.data(), "gby");
				//printf("%04d\n", tmpGlobalX);
...
int main(int argc, const char **argv) {
	...
	pthread_t threads[1];
	int rc;
	//rc = pthread_create(&threads[0], NULL, detect, (void *)rc);
	rc = pthread_create(&threads[0], NULL, detect, (void *)argv[1]);
	if (rc){
