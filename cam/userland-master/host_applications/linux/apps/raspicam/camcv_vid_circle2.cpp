...
Mat gray,frame,face,face_resized, hsvMat;
vector<Mat> images;
vector<int> labels;
...
int nCount=0;
IplImage *py, *pu, *pv, *pu_big, *pv_big, *image,* dstImage, *hsvImage;
...
static void default_status(RASPIVID_STATE *state)
{
	...
	state->timeout 			= 600000000;     // capture time : here 65 s
	state->width 			= 320;      // use a multiple of 320 (640, 1280)
	state->height 			= 240;		// use a multiple of 240 (480, 960)
	state->bitrate 			= 1700000; // This is a decent default bitrate for 1080p
	...
	if (pData)
	{
		...
		if (buffer->length)
		{
			...
			if (pData->pstate->graymode==0)
			{
				...
				cvCvtColor(dstImage,hsvImage,CV_RGB2HSV);	// convert in HSV color space (slow)
				hsvMat = cvarrToMat(hsvImage);
				gray = cvarrToMat(dstImage);

				//hsvMat.copyTo(gray);
				inRange(hsvMat, Scalar(iLowH, iLowS, iLowV), Scalar(iHighH, iHighS, iHighV), hsvMat);
				erode(hsvMat, hsvMat, getStructuringElement(MORPH_ELLIPSE, Size(5, 5)) );
				dilate(hsvMat, hsvMat, getStructuringElement(MORPH_ELLIPSE, Size(5, 5)) ); 
				Moments oMoments = moments(hsvMat);

				double dM01 = oMoments.m01;
				double dM10 = oMoments.m10;
				double dArea = oMoments.m00;
				double dRadius = sqrt(dArea/ 255 / 3.14159265358979323846);
				if (dArea < 10*10*255) {
				} else {
					...
					if(noDisplay == false) {
						...
						circle( gray, center, 5, Scalar(0,255,0), -1, 8, 0 );
						circle( gray, center, r, Scalar(0,255,0), 1, 8, 0 );
					}
					zmq::message_t message(64);
					*((int*)message.data())     = posX;
					*(((int*)message.data())+1) = posY;
					*(((int*)message.data())+2) = r;
...
void * detect(void * pData)
{
	...
	//fn_haar = (char*)pData;//"/home/pi/Desktop/lbpcascade_frontalface.xml";
	// load face model
	...
	hsvImage = cvCreateImage(cvSize(w,h), IPL_DEPTH_8U, 3);	// hsv picture to display
...
void make_hsv_adjustment() {
	cvNamedWindow("Control", CV_WINDOW_AUTOSIZE);
	cvNamedWindow("HSV_inRange", CV_WINDOW_AUTOSIZE);
...
int main(int argc, const char **argv) {
	...
	publisher.bind ("ipc://detect.ipc");

	pthread_mutex_init(&mutex, NULL);
	pthread_t threads[1];
	int rc;
	rc = pthread_create(&threads[0], NULL, detect, NULL);
	if (rc){
